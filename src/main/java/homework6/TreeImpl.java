...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    public boolean isBalanced(Node node) {  // Добавил к коду урока 6 метод тз задания. Никак не могу понять как передать в метод node из Test
        return (node == null) ||
    ...
    private NodeAndParent doFind(E value) {
        Node<E> current = root;
        Node<E> parent = null;
        int level = 0; //добавил к коду урока 6

        while (current != null) {
            level++; //добавил к коду урока 6
            if (current.getValue().equals(value)) {
                return new NodeAndParent(current, parent, level); //добавил к коду урока 6
            }
        ...
        return new NodeAndParent(null, parent, level); //добавил к коду урока 6
    }

    public int getLevel(E value) {  //добавил к коду урока 6
        NodeAndParent nodeAndParent = doFind(value);
        return nodeAndParent.level;
    }
